...
## pagoda is removed from this tutorial as it was not possible to install in time.
if (org == "human") {
...
hist(log10(reads_per_cell+1),main='reads per cell',col='wheat',breaks = 100)
hist(log10(genes_per_cell+1), main='genes per cell', col='wheat',breaks = 100)
plot(reads_per_cell, genes_per_cell, log='xy', col='wheat')
hist(log10(reads_per_gene+1),main='reads per gene',col='wheat',breaks = 100)
```
...
MAX_PCT_MITO = 16   ## user-defined setting
...
pbmc <- CreateSeuratObject(counts = myCountMatrixSparse, project = "pbmc3k", min.cells = 3, min.features = 200)
pbmc
```
...
Question: how to identify cells that are suspicious in one of these plots?
...
cluster1.markers <- FindMarkers(pbmc, ident.1 = 3, min.pct = 0.25)
...
cluster5.markers <- FindMarkers(pbmc, ident.1 = c(5,6), ident.2 = c(0,1,2,3,4), min.pct = 0.25)
...
```{r}
pbmc.markers[pbmc.markers$cluster==5 & pbmc.markers$gene=="CD74",]
pbmc.markers["CD741",]
```
...
```{r}
VlnPlot(pbmc, features = c("MS4A1", "CD79A", "CD74"))

VlnPlot(pbmc, features = c("MS4A1", "IGKC", "HLA-DRA"))


VlnPlot(pbmc, features = c("TCL1A", "IGHD", "IGHA1"))
...
```{r}
VlnPlot(pbmc, features = c("NKG7", "GNLY"), slot = "counts", log = TRUE)
...
```{r}
new.cluster.ids <- c("Dendritic", "NK", "Naive CD4 T", "Memory CD4 T", "CD14+ Mono", "B", "CD8 T")
